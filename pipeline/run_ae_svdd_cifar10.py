...
# (STILE IDENTICO a run_ae_svdd.py: run dirs, W&B, 4 fasi, wrapper encoder)
# ============================================================
import os
...
# ---> usa esattamente la DataModule che hai incollato
from utils.data_cifar import CIFAR10OneClassDataModule
...
def run_pipeline(cfg: Config) -> None:
    ...
    # per CIFAR-10 riuso cfg.normal_digit come "normal_class" (0..9)
    normal_class = int(cfg.normal_digit)
    ...
    # ---------------- DataModule (CIFAR-10 One-Class) ----------------
    # NB: la tua DataModule restituisce già y binario (0 normal, 1 anomaly)
    dm = CIFAR10OneClassDataModule(
        data_dir=cfg.data_dir,
        normal_class=normal_class,
        pollution_rate=cfg.pollution_rate,
        batch_size=int(_get(cfg, "svdd_batch_size", 200)),  # paper-like: 200
    ...
    if os.path.exists(ae_ckpt_path):
        ...
        if cfg.ae_mode == "rcae":
            rcae_cfg = RCAEConfig(
                ...
                batch_size=int(_get(cfg, "ae_batch_size", 200)),  # paper-like: 200
    ...
    svdd_lr = float(_get(cfg, "svdd_lr", 1e-4))  # opzionale, se in futuro lo usi
    svdd_weight_decay = float(_get(cfg, "svdd_weight_decay", 1e-6))  # paper-like: 1e-6
    ...
    enc_trained, c, R, svdd_hist = train_deepsvdd(
        ...
        nu=svdd_nu,
        epochs=svdd_epochs,
        lr=svdd_lr,  # tenuto per compatibilità firma
    ...
    # NB: i tuoi label sono già binari -> non serve normal_class qui
    ...
    X_test, _y_test = collect_images_and_labels(test_loader)
    plot_extremes_in_class(
        images=X_test,
        anom_scores=scores,
        labels=labels,  # binarie
        out_dir=plots_dir,
    ...
    print(f"[DONE] DeepSVDD CIFAR-10 | AUROC={auc:.4f} | AP={ap:.4f} | R={float(R):.6f}")
